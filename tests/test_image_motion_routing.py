...
class ImageMotionRoutingTests(unittest.TestCase):
    def workflow(self, component_submit, component_poll):
        storage = MemoryStorage()
        asset = component_asset()
        storage.json[CREATIVE_ASSETS_KEY] = [asset]
        storage.json[IMAGE_MOTION_JOBS_KEY] = [component_job()]
        ...
        workflow.render_fallback = lambda *args, **kwargs: self.fail("component motion must not use stable fallback")
