...
docker run --rm -p 8501:8501 `
  ...
  -e BEDROCK_MODEL_ID="anthropic.claude-fable-5" `
  -e BEDROCK_MODEL_FALLBACK_IDS="anthropic.claude-opus-4-8,eu.anthropic.claude-sonnet-4-5-20250929-v1:0" `
...
4. 确认目标模型可用：默认主模型是 `anthropic.claude-fable-5`，fallback 为 `anthropic.claude-opus-4-8,eu.anthropic.claude-sonnet-4-5-20250929-v1:0`。
5. 如果该 profile 不可用，选择账号已可用的 Claude 模型/profile，并把 `BEDROCK_MODEL_ID` 或 `BEDROCK_MODEL_FALLBACK_IDS` 改成对应 ID。
...
export BEDROCK_MODEL_FALLBACK_IDS=anthropic.claude-opus-4-8,eu.anthropic.claude-sonnet-4-5-20250929-v1:0
...
| `BEDROCK_MODEL_ID` | Environment | `anthropic.claude-fable-5` |
| `BEDROCK_MODEL_FALLBACK_IDS` | Environment | `anthropic.claude-opus-4-8,eu.anthropic.claude-sonnet-4-5-20250929-v1:0` |
