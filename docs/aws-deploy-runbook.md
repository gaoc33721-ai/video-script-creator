...
docker run --rm -p 8501:8501 `
  ...
  -e BEDROCK_MODEL_FALLBACK_IDS="eu.amazon.nova-pro-v1:0" `
...
4. 确认目标模型可用：默认是 `eu.anthropic.claude-sonnet-4-5-20250929-v1:0`，并保留 `eu.amazon.nova-pro-v1:0` 作为 fallback。
5. 如果该 profile 不可用，选择账号已可用的 Claude 或 Amazon Nova 模型/profile，并把 `BEDROCK_MODEL_ID` 或 `BEDROCK_MODEL_FALLBACK_IDS` 改成对应 ID。
...
export BEDROCK_MODEL_FALLBACK_IDS=eu.amazon.nova-pro-v1:0
...
| `BEDROCK_MODEL_ID` | Environment | `eu.anthropic.claude-sonnet-4-5-20250929-v1:0` |
| `BEDROCK_MODEL_FALLBACK_IDS` | Environment | `eu.amazon.nova-pro-v1:0` |
